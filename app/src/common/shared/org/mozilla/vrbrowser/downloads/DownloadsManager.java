...
public class DownloadsManager {
    ...
    public interface DownloadsListener {
        ...
        default void onDownloadError(@NonNull String error) {}
    }
    ...
    public void startDownload(@NonNull DownloadJob job) {
        if (!URLUtil.isHttpUrl(job.getUri()) && !URLUtil.isHttpsUrl(job.getUri())) {
            notifyDownloadError("Cannot download non http/https files");
            return;
        }
        ...
        if (SettingsStore.getInstance(mContext).getDownloadsStorage() == SettingsStore.EXTERNAL) {
            ...
            if (outputPath == null) {
                notifyDownloadError("Cannot create output file");
                return;
            }
    ...
    private void notifyDownloadError(@NonNull String error) {
        mListeners.forEach(listener -> listener.onDownloadError(error));
    }
